...
export function decodeHtmlEntities(value?: string | null) {
  return (value || "")
    ...
    .replace(/&szlig;/gi, "ß")
    // Latin-1 named accents commonly seen from WordPress content
    .replace(/&(eacute|Eacute);/g, (m, p1) => p1[0] === 'E' ? 'É' : 'é')
    .replace(/&(aacute|Aacute);/g, (m, p1) => p1[0] === 'A' ? 'Á' : 'á')
    .replace(/&(oacute|Oacute);/g, (m, p1) => p1[0] === 'O' ? 'Ó' : 'ó')
    .replace(/&(uacute|Uacute);/g, (m, p1) => p1[0] === 'U' ? 'Ú' : 'ú')
    .replace(/&(igrave|Igrave);/g, (m, p1) => p1[0] === 'I' ? 'Ì' : 'ì')
    .replace(/&(egrave|Egrave);/g, (m, p1) => p1[0] === 'E' ? 'È' : 'è')
    .replace(/&(agrave|Agrave);/g, (m, p1) => p1[0] === 'A' ? 'À' : 'à')
    .replace(/&(ograve|Ograve);/g, (m, p1) => p1[0] === 'O' ? 'Ò' : 'ò')
    .replace(/&(ugrave|Ugrave);/g, (m, p1) => p1[0] === 'U' ? 'Ù' : 'ù')
    .replace(/&(ecirc|Ecirc);/g, (m, p1) => p1[0] === 'E' ? 'Ê' : 'ê')
    .replace(/&(acirc|Acirc);/g, (m, p1) => p1[0] === 'A' ? 'Â' : 'â')
    .replace(/&(ocirc|Ocirc);/g, (m, p1) => p1[0] === 'O' ? 'Ô' : 'ô')
    .replace(/&(ucirc|Ucirc);/g, (m, p1) => p1[0] === 'U' ? 'Û' : 'û')
    .replace(/&(euml|Euml);/g, (m, p1) => p1[0] === 'E' ? 'Ë' : 'ë')
    .replace(/&(iuml|Iuml);/g, (m, p1) => p1[0] === 'I' ? 'Ï' : 'ï')
    .replace(/&(ccedil|Ccedil);/g, (m, p1) => p1[0] === 'C' ? 'Ç' : 'ç')
    .replace(/&#038;/g, "&")
    .replace(/&#8211;/g, "–")
    .replace(/&#8212;/g, "—")
    .replace(/&#8217;/g, "’")
